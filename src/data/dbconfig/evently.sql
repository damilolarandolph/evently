...
CREATE TABLE `database_state` (
  `should_hoist` bool DEFAULT false
);
...
ALTER TABLE `organizer` ADD FOREIGN KEY (`user`) REFERENCES `user` (`email`);

ALTER TABLE `person` ADD FOREIGN KEY (`user`) REFERENCES `user` (`email`);

ALTER TABLE `event` ADD FOREIGN KEY (`type`) REFERENCES `event_type` (`id`);

ALTER TABLE `event` ADD FOREIGN KEY (`category`) REFERENCES `event_category` (`id`);

ALTER TABLE `event` ADD FOREIGN KEY (`organizer`) REFERENCES `organizer` (`user`);

ALTER TABLE `reservation` ADD FOREIGN KEY (`event`) REFERENCES `event` (`id`);

ALTER TABLE `reservation` ADD FOREIGN KEY (`person`) REFERENCES `person` (`user`);
